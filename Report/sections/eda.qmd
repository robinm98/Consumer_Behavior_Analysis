...
## Data Distribution Predictive Features (X)
Understanding the distribution of predictive features is essential for gaining insights into the characteristics and variability of our dataset. In this section, we will examine the data distribution of key predictive features (X) that influence passenger preferences for air travel upgrades.

This analysis will help us understand how these features interact and contribute to predicting passenger choices for additional services, thereby enhancing the accuracy and reliability of our predictive models.
...
**Monday:** Has the highest number of flights, with counts slightly above 8,000. This suggests that Monday is the busiest day for flights.
...
Extra Baggage Rates: Negative trend; advance bookers less likely to want extra baggage.

In-Flight Meals Rates: Negative trend; advance bookers less likely to want in-flight meals.

Preferred Seats Rates: Negative trend; advance bookers less likely to select preferred seats.
```{r, echo = FALSE, message = FALSE, warning=FALSE}
...
The first set of graphs compares the proportion of passengers wanting extra baggage and in-flight meals based on their preferred seat selection. The left graph shows that a higher proportion of passengers with preferred seats (1) want extra baggage compared to those without preferred seats (0). Similarly, the right graph indicates that passengers with preferred seats are also more likely to want in-flight meals than those without preferred seats. This suggests a correlation between the desire for preferred seating and the preference for additional services such as extra baggage and in-flight meals.
```{r, echo = FALSE, message = FALSE, warning=FALSE}
...
The second set of graphs examines the proportion of passengers wanting extra baggage and preferred seats based on their in-flight meal preferences. The left graph demonstrates that passengers who opt for in-flight meals (1) have a higher proportion of wanting extra baggage than those who do not (0). The right graph reveals that passengers wanting in-flight meals are also more likely to choose preferred seats. This indicates that passengers' choice for in-flight meals is associated with a higher likelihood of selecting other additional services, such as extra baggage and preferred seats.
```{r, echo = FALSE, message = FALSE, warning=FALSE}
